...
const char* BUILD_VERSION = "MAZE-TOWER50-V1-2026-07-29";
...
std::vector<SDL_Texture*> wallTextures(50, nullptr);
std::vector<SDL_Texture*> floorTextures(5, nullptr);
Mix_Music* explorationBgm = nullptr;
Mix_Music* homeBgm = nullptr;
Mix_Chunk* playerFootstepSfx = nullptr;
Mix_Chunk* enemyFootstepSfx = nullptr;
Mix_Chunk* keyPickupSfx = nullptr;
Mix_Chunk* itemPickupSfx = nullptr;
Mix_Chunk* floorClearSfx = nullptr;
Mix_Chunk* gameClearSfx = nullptr;
Mix_Chunk* damageSfx = nullptr;
Uint32 nextPlayerFootstepAt = 0;
Uint32 nextEnemyFootstepAt = 0;
...
bool mobileCrouchingHeld = false;
bool mobileRunHeld = false;
...
void updatePlayer(double deltaTime) {
    ...
    playerCrouching = (keys[SDL_SCANCODE_LSHIFT] || keys[SDL_SCANCODE_RSHIFT] || mobileCrouchingHeld) && playerOnGround;
    ...
    bool runningForward = !playerCrouching && playerOnGround &&
        (((keys[SDL_SCANCODE_UP] && ctrlHeld) ||
          (keys[SDL_SCANCODE_W] && keys[SDL_SCANCODE_Q])) || mobileRunning || mobileRunHeld);
...
void draw3DView(SDL_Renderer* renderer) {
    ...
    for (int y = 0; y < horizonY; ++y) {
        double t = horizonY > 1 ? static_cast<double>(y) / horizonY : 0.0;
        Uint8 r = static_cast<Uint8>(55 + 75 * t);
        Uint8 g = static_cast<Uint8>(105 + 85 * t);
        Uint8 b = static_cast<Uint8>(170 + 70 * t);
    ...
    for (int screenX = 0; screenX < SCREEN_WIDTH; screenX++) {
        ...
        int rawWallTop = horizonY - wallHeight / 2;
        int rawWallBottom = horizonY + wallHeight / 2;
        int wallTop = std::max(0, rawWallTop);
        int wallBottom = std::min(SCREEN_HEIGHT - 1, rawWallBottom);
        ...
        if (wallTexture != nullptr && wallTextureWidth > 0) {
            ...
            // 画面外へはみ出した部分だけテクスチャも切り取り、横壁の歪みを防ぐ。
            int visibleHeight = std::max(1, wallBottom - wallTop + 1);
            int sourceY = 0;
            int sourceH = wallTextureHeight;
            if (wallHeight > 0) {
                double topRatio = static_cast<double>(wallTop - rawWallTop) / wallHeight;
                double visibleRatio = static_cast<double>(visibleHeight) / wallHeight;
                sourceY = std::clamp(static_cast<int>(topRatio * wallTextureHeight), 0, wallTextureHeight - 1);
                sourceH = std::clamp(static_cast<int>(visibleRatio * wallTextureHeight), 1, wallTextureHeight - sourceY);
            }
            SDL_Rect source = {textureX, sourceY, 1, sourceH};
            SDL_Rect destination = {screenX, wallTop, 1, visibleHeight};
...
bool tryPickUpKey() {
    for (auto& key : keysInMaze) {
        ...
        if (lookingAt(key.x, key.y, 1.65, 0.22)) {
            key.available = false;
            key.collected = true;
            if (keyPickupSfx != nullptr) Mix_PlayChannel(-1, keyPickupSfx, 0);
...
void updateTrapsAndItems() {
    ...
    for (auto& trap : floorTraps) {
        if (!trap.usedByPlayer && jumpHeight < 0.22 && std::hypot(playerX - trap.x, playerY - trap.y) < 0.34) {
            ...
            if (now >= playerDamageCooldownUntil) {
                playerHearts--;
                if (damageSfx != nullptr) Mix_PlayChannel(-1, damageSfx, 0);
    ...
    for (auto& item : randomItems) {
        ...
        if (itemPickupSfx != nullptr) Mix_PlayChannel(-1, itemPickupSfx, 0);
        switch (item.effect) {
            ...
            case ItemEffect::DAMAGE:
                if (now >= playerDamageCooldownUntil) { playerHearts--; playerDamageCooldownUntil = now + 2000; if (damageSfx != nullptr) Mix_PlayChannel(-1, damageSfx, 0); }
...
void drawWorldObjects(SDL_Renderer* renderer) {
    projectFloorMarker(renderer, 1.5, 1.5, {50,150,255,185}, 1.15);
...
void drawMobileControls(
    ...
    drawButton(renderer, font, mobileCrouchButton, mobileCrouchingHeld);
    drawButton(renderer, font, mobileJumpButton, false);
    drawButton(renderer, font, mobileInteractButton, false);
    drawButton(renderer, font, mobileRunButton, mobileRunHeld);
}
...
void saveGame() {
    ...
    if (!file) { showActionMessage("セーブに失敗しました"); return; }
    file << "MAZE50_SAVE_V2\n";
    file << currentFloor << ' ' << static_cast<int>(selectedDifficulty) << ' '
         << score << ' ' << highestScore << ' ' << playerHearts << '\n';
    file << MAZE_CELLS << ' ' << MAZE_WIDTH << ' ' << MAZE_HEIGHT << '\n';
    file << playerX << ' ' << playerY << ' ' << playerAngle << ' ' << cameraPitch << '\n';
    file << goalX << ' ' << goalY << '\n';
    file << keysInMaze.size() << '\n';
    for (const auto& key : keysInMaze) {
        file << key.x << ' ' << key.y << ' ' << key.available << ' ' << key.collected << ' '
             << static_cast<int>(key.color.r) << ' ' << static_cast<int>(key.color.g) << ' '
             << static_cast<int>(key.color.b) << ' ' << key.name << '\n';
    }
    for (int y=0; y<MAZE_HEIGHT; ++y) { for (int x=0; x<MAZE_WIDTH; ++x) file << maze[y][x]; file << '\n'; }
    for (int y=0; y<MAZE_HEIGHT; ++y) { for (int x=0; x<MAZE_WIDTH; ++x) file << (visited[y][x]?1:0); file << '\n'; }
    saveMessageUntil = SDL_GetTicks() + 1800;
    showActionMessage("セーブしました");
}

bool loadGame() {
    std::ifstream file("maze_save.txt");
    if (!file) { showActionMessage("セーブデータがありません"); return false; }
    std::string header; std::getline(file, header);
    if (header != "MAZE50_SAVE_V2") { showActionMessage("古いセーブ形式です"); return false; }
    int difficultyValue=2;
    file >> currentFloor >> difficultyValue >> score >> highestScore >> playerHearts;
    selectedDifficulty = static_cast<Difficulty>(std::clamp(difficultyValue,0,4));
    file >> MAZE_CELLS >> MAZE_WIDTH >> MAZE_HEIGHT;
    file >> playerX >> playerY >> playerAngle >> cameraPitch;
    file >> goalX >> goalY;
    size_t keyCount=0; file >> keyCount;
    keysInMaze.clear();
    for (size_t i=0;i<keyCount;++i) {
        KeyItem key; int r,g,b; file >> key.x >> key.y >> key.available >> key.collected >> r >> g >> b >> key.name;
        key.color = {static_cast<Uint8>(r),static_cast<Uint8>(g),static_cast<Uint8>(b),255}; keysInMaze.push_back(key);
    }
    maze.assign(MAZE_HEIGHT,std::vector<int>(MAZE_WIDTH,1));
    visited.assign(MAZE_HEIGHT,std::vector<bool>(MAZE_WIDTH,false));
    std::string row;
    for (int y=0;y<MAZE_HEIGHT;++y) { file >> row; if ((int)row.size()<MAZE_WIDTH) return false; for(int x=0;x<MAZE_WIDTH;++x) maze[y][x]=row[x]-'0'; }
    for (int y=0;y<MAZE_HEIGHT;++y) { file >> row; if ((int)row.size()<MAZE_WIDTH) return false; for(int x=0;x<MAZE_WIDTH;++x) visited[y][x]=(row[x]=='1'); }
    enemyAlive=false; extraChasers.clear(); patrolEnemies.clear(); floorTraps.clear(); randomItems.clear();
    if (currentFloor>=1 && currentFloor<=50 && wallTextures[currentFloor-1]) wallTexture=wallTextures[currentFloor-1];
    if (wallTexture) SDL_QueryTexture(wallTexture,nullptr,nullptr,&wallTextureWidth,&wallTextureHeight);
    gameState = GameState::GAME; overlayState=OverlayState::NONE; mapOpen=false;
    if (explorationBgm) Mix_PlayMusic(explorationBgm,-1);
    showActionMessage("ロードしました");
    return true;
}
...
void drawHome(
    ...
    SDL_SetRenderDrawColor(renderer, 15, 20, 32, 255);
    SDL_RenderClear(renderer);
    if (homeTexture != nullptr) {
        SDL_Rect dst{0, 0, SCREEN_WIDTH, SCREEN_HEIGHT};
        SDL_RenderCopy(renderer, homeTexture, nullptr, &dst);
    ...
    drawButton(renderer, buttonFont, loadButton);
...
void drawClear(
    ...
    drawText(
        renderer,
        titleFont,
        currentFloor >= MAX_FLOOR ? "GAME CLEAR" : "第" + std::to_string(currentFloor) + "層クリア",
        SCREEN_WIDTH / 2,
        235,
        green
    );
    ...
    drawText(
        renderer,
        smallFont,
        currentFloor >= MAX_FLOOR ? "Enter：最初から" : "Enter：次の層へ",
        SCREEN_WIDTH / 2,
        420
    );
}
...
void startGame() {
    ...
    score = 0;
    generateMaze();
    if (!wallTextures.empty() && wallTextures[0] != nullptr) wallTexture = wallTextures[0];
    if (explorationBgm != nullptr) Mix_PlayMusic(explorationBgm, -1);
...
void startNextFloor() {
    if (currentFloor < MAX_FLOOR) {
        ++currentFloor;
        score += 500;
        MAZE_CELLS = getDifficultyConfig(selectedDifficulty).cells + (currentFloor - 1) * 2;
        MAZE_CELLS = std::min(MAZE_CELLS, 45);
        if (wallTextures[currentFloor - 1] != nullptr) wallTexture = wallTextures[currentFloor - 1];
        generateMaze();
        setMouseCaptureForGameplay();
    } else {
        score += 500;
        highestScore = std::max(highestScore, score);
        if (gameClearSfx != nullptr) Mix_PlayChannel(-1, gameClearSfx, 0);
    }
}
...
void returnHome() {
    ...
    SDL_SetRelativeMouseMode(SDL_FALSE);
    if (homeBgm != nullptr) Mix_PlayMusic(homeBgm, -1);
}
...
int runGame() {
    ...
    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) != 0) {
        std::cerr << "SDL_mixerの初期化に失敗: " << Mix_GetError() << std::endl;
    }
    ...
    for (int i = 0; i < 50; ++i) {
        char path[128];
        std::snprintf(path, sizeof(path), "assets/walls/wall_%02d.png", i + 1);
        wallTextures[i] = IMG_LoadTexture(renderer, path);
    }
    wallTexture = wallTextures[0];
    enemyTexture = IMG_LoadTexture(renderer, "assets/robots/robot_blue_transparent.png");
    homeTexture = IMG_LoadTexture(renderer, "assets/ui/home.png");
    goalTexture = IMG_LoadTexture(renderer, "assets/ui/goal_green.png");
    startTexture = IMG_LoadTexture(renderer, "assets/ui/start_blue.png");
    const char* floorPaths[5] = {
        "assets/floors/floor_01_stone.png", "assets/floors/floor_02_tile.png",
        "assets/floors/floor_03_concrete.png", "assets/floors/floor_04_wood.png",
        "assets/floors/floor_05_metal.png"
    };
    for (int i = 0; i < 5; ++i) floorTextures[i] = IMG_LoadTexture(renderer, floorPaths[i]);
    explorationBgm = Mix_LoadMUS("assets/audio/maze3d_bgm_exploration_01.wav");
    homeBgm = Mix_LoadMUS("assets/audio/home_bgm_01.wav");
    playerFootstepSfx = Mix_LoadWAV("assets/audio/footstep_stone_01.wav");
    enemyFootstepSfx = Mix_LoadWAV("assets/audio/enemy_footstep_01.wav");
    keyPickupSfx = Mix_LoadWAV("assets/audio/key_pickup_01.wav");
    itemPickupSfx = Mix_LoadWAV("assets/audio/item_pickup_01.wav");
    floorClearSfx = Mix_LoadWAV("assets/audio/floor_clear_01.wav");
    gameClearSfx = Mix_LoadWAV("assets/audio/game_clear_01.wav");
    damageSfx = Mix_LoadWAV("assets/audio/damage_hit_01.wav");
    ...
    Button loadButton = {{350, 355, 300, 65}, "ロード"};
    Button homeSettingsButton = {{350, 440, 300, 65}, "設定"};
    Button rulesButton = {{350, 525, 300, 65}, "ルール"};
        ...
        while (SDL_PollEvent(&event)) {
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (tryUseKeyAtGoal()) {
                        ...
                        if (currentFloor >= MAX_FLOOR) {
                            ...
                            highestScore = std::max(highestScore, score);
                            if (gameClearSfx != nullptr) Mix_PlayChannel(-1, gameClearSfx, 0);
                        } else if (floorClearSfx != nullptr) {
                            Mix_PlayChannel(-1, floorClearSfx, 0);
                        }
            ...
            if (event.type == SDL_FINGERDOWN) {
                ...
                if (
                    ...
                    !pointInside(mobilePauseButton.rect, pointerX, pointerY) &&
                    !pointInside(mobileJumpButton.rect, pointerX, pointerY) &&
                    !pointInside(mobileInteractButton.rect, pointerX, pointerY) &&
                    !pointInside(mobileRunButton.rect, pointerX, pointerY)
                ) {
            ...
            if (pointerDown) {
                ...
                if (overlayState == OverlayState::DIFFICULTY) {
                    ...
                    } else if (pointInside(loadButton.rect, pointerX, pointerY)) {
                        if (loadGame()) { SDL_SetRelativeMouseMode(SDL_TRUE); }
                    } else if (
                        pointInside(
                    ...
                    if (controlMode == ControlMode::MOBILE) {
                        if (
                            ...
                            mapOpen = !mapOpen;
                            setMouseCaptureForGameplay();
                        } else if (pointInside(mobileCrouchButton.rect, pointerX, pointerY)) {
                            mobileCrouchingHeld = true;
                        } else if (pointInside(mobileJumpButton.rect, pointerX, pointerY)) {
                            startPlayerJump();
                        } else if (pointInside(mobileRunButton.rect, pointerX, pointerY)) {
                            mobileRunHeld = true;
                        } else if (pointInside(mobileInteractButton.rect, pointerX, pointerY)) {
                            tryPickUpKey();
                            if (tryUseKeyAtGoal()) {
                                SDL_SetRelativeMouseMode(SDL_FALSE);
                                if (currentFloor >= MAX_FLOOR) {
            ...
            if (
                ...
                if (gameState == GameState::CLEAR && key == SDLK_RETURN) {
                    if (currentFloor >= MAX_FLOOR) startGame();
                    else startNextFloor();
                    gameState = GameState::GAME;
                } else if (gameState == GameState::GAME_OVER && key == SDLK_RETURN) {
                    startGame();
                    gameState = GameState::GAME;
                }
            }
    ...
    if (homeTexture != nullptr) SDL_DestroyTexture(homeTexture);
    if (goalTexture != nullptr) SDL_DestroyTexture(goalTexture);
    if (startTexture != nullptr) SDL_DestroyTexture(startTexture);
    for (auto* texture : wallTextures) if (texture != nullptr) SDL_DestroyTexture(texture);
    for (auto* texture : floorTextures) if (texture != nullptr) SDL_DestroyTexture(texture);
    if (explorationBgm != nullptr) Mix_FreeMusic(explorationBgm);
    if (homeBgm != nullptr) Mix_FreeMusic(homeBgm);
    if (playerFootstepSfx != nullptr) Mix_FreeChunk(playerFootstepSfx);
    if (enemyFootstepSfx != nullptr) Mix_FreeChunk(enemyFootstepSfx);
    if (keyPickupSfx != nullptr) Mix_FreeChunk(keyPickupSfx);
    if (itemPickupSfx != nullptr) Mix_FreeChunk(itemPickupSfx);
    if (floorClearSfx != nullptr) Mix_FreeChunk(floorClearSfx);
    if (gameClearSfx != nullptr) Mix_FreeChunk(gameClearSfx);
    if (damageSfx != nullptr) Mix_FreeChunk(damageSfx);
    Mix_CloseAudio();
    SDL_DestroyRenderer(renderer);
    SDL_DestroyWindow(window);
